    ...
    for group in groups:
        if len(group) >= crowd_threshold:
            ...
            if crowd_timer >= save_duration_threshold * cap.get(cv2.CAP_PROP_FPS):
                ...
                signed_url = bucket.blob(output_filename).generate_signed_url(expiration=datetime.timedelta(hours=76))
                ...
                crowd_entry = {'camName':"6r",'image': encoded_image, 'crowdDuration': f"{hours} hours {minutes} minutes {seconds} seconds", 'crowdStartTime': crowd_start_time, 'crowdEndTime': crowd_end_time, 'noOfPersons': person, 'detetctedVideo': signed_url }
